...
import style from './style/tableBodyGroup.module.css';
import { useNavigate } from 'react-router-dom';

const TableBodyGroup = ({ data }) => {
          ...
          <td>
            <div className={style.context_row}>
              ...
              <div onClick={() => handleOpenProject(item.id)}>
                <span className={style.row_text}>{item.nameProject}</span>
              </div>
            </div>
          </td>
          <td>
        ...
        <React.Fragment key={group}>
          <tr onClick={() => handleGroupClick(group)}>
            <td colSpan="8">
              <div className={style.context_row}>
                <input type="checkbox" />
                <span className={style.row_group_text}>{group}</span>
              </div>
            </td>
          </tr>
          {expandedRows[group] &&
