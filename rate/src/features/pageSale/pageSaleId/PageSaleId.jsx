...
import { dataforsale } from '../../../mock/index.js'
import style from './style/pageSaleId.module.css'

import chevronLeft from '../../../assets/chevron_left.svg'

import TableItemsServices from './tableItemsServices/TableItemsServices.jsx'
import TableReceipts from './tableReceipts/TableReceipts.jsx'
import TableExpenses from './tableExpenses/TableExpenses.jsx'
import TableShipment from './tableShipment/TableShipment.jsx'
import TableBill from './tableBill/TableBill.jsx'
...
const PageSaleId = () => {
	...
	useEffect(() => {
		const sale = dataforsale.find(el => el.id === Number(id))
		setSaleItem(sale)
	}, [id])

	const sortDate = dataforsale.sort((a, b) => new Date(b.date) - new Date(a.date))

	const formattedDataforsale = sortDate.map(item => {
	...
	formattedDataforsale.forEach(item => {
	...
	return (
		<>
			...
			<div className={style.content_container}>
				<div className={style.container_saleId}>
					<div className={style.container_saleId_btn} onClick={() => navigate(-1)}>
						<img src={chevronLeft} alt='chevronLeft' />
						<span className={style.container_saleId_btn_text}>Все проекты</span>
					</div>
					<div className={style.container_content_sale}>
						<div className={style.content_header_left}>
							...
							<div className={style.content_header_left_date}>
								<div className={style.content_header_left_date_text}>
									<span>{saleItem.startDate}</span>-<span>{saleItem.endDate}</span>
								</div>
								<div>?</div>
							</div>
						</div>
					...
					<div className={style.container_bar}>
								...
								<div className={style.container_bar_block_one_data_main}>
									<div>
										<span className={style.container_bar_block_one_data_amount}>
											{saleItem.transAmount} <span>₽</span>
										</span>
									</div>
								</div>
							...
							<div className={style.container_bar_block_one_data}>
								<div>
									<div className={style.container_bar_block_info}>
										<div>
											Тип: <span>{saleItem.type}</span>
										</div>
										<br />
										<div>
											Клиент: <span>{saleItem.client}</span>
										</div>
										<br />
										<div>
											{' '}
											Дата: <span>{saleItem.date}</span>
										</div>
									</div>
								</div>
							</div>
						...
						<div className={style.container_bar_block_one}>
							...
							<div className={style.vertical_divider}></div>
							<div className={style.container_bar_block_one_data}>
								<div className={style.profit_bar}>Доход: {saleItem.profit}</div>
								<div className={style.gane_bar}>Расход: -{saleItem.gane}</div>
							</div>
						</div>
					</div>
					...
					<div className={style.table_control}>
						<div className={style.table_select_modal}>
							<div className={style.selection_modal}>
								...
								<span
									className={`${style.select_item2} ${
										activeTable === 'receipts' ? style.select_item2_active : ''
									}`}
								...
								<span
									className={`${style.select_item2} ${
										activeTable === 'expenses' ? style.select_item2_active : ''
									}`}
								...
								<span
									className={`${style.select_item2} ${
										activeTable === 'shipment' ? style.select_item2_active : ''
									}`}
									onClick={() => setActiveTable('shipment')}
								>
									Отгрузки
								</span>
								<span
									className={`${style.select_item3} ${
										activeTable === 'bill' ? style.select_item2_active : ''
									}`}
									onClick={() => setActiveTable('bill')}
								>
									Счета
								</span>
							</div>
							<div className={style.add_item_service} onClick={handleNewProject}>
								...
								{activeTable === 'receipts' && 'Добавить поступления'}
								{activeTable === 'expenses' && 'Добавить расходы'}
								{activeTable === 'shipment' && 'Добавить отгрузки'}
								{activeTable === 'bill' && 'Добавить счёт'}
								{activeTable === 'fileComment' && ''}
							</div>
						</div>
					...
					{activeTable === 'receipts' && <TableReceipts />}
					{activeTable === 'expenses' && <TableExpenses />}
					{activeTable === 'shipment' && <TableShipment />}
					{activeTable === 'bill' && <TableBill />}
